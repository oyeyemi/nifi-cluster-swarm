...
# Values are quoted deliberately: this file gets \`source\`d directly for
# Swarm deploys (docker stack deploy doesn't auto-load .env the way
# docker compose does), and \`source\` executes this file as a real bash
# script - an unquoted value containing spaces (like the identity DNs
# below) gets silently word-split into several assignments instead of
# staying as one value. Quoting prevents that.
